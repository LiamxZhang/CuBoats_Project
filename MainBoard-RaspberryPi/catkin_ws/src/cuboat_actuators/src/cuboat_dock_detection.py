        ...
        while not rospy.is_shutdown():
            ...
            if len(ser_info) == 5 and ser_info[-1]=="\n" :
                for j in range(len(ser_info)-1):
                    ...
                    self.dock_servo_state[j] = bool(int(ser_info[j]))
            # publish
            rospy.loginfo('Magnetic dock: %d, %d, %d, %d.', self.dock_servo_state[0], self.dock_servo_state[2], self.dock_servo_state[1], self.dock_servo_state[3])
            self.dock_detector.publish(dock(self.dock_servo_state[0], self.dock_servo_state[2], self.dock_servo_state[1], self.dock_servo_state[3]))
            self.rate.sleep()
