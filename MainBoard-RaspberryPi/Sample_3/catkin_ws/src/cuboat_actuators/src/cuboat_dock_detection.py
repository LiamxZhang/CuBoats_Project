...
class CuBoat_Dock_Detector:
    ...
    def publish(self):
        while not rospy.is_shutdown():
            ...
            # publish
            rospy.loginfo('Magnetic dock: %d, %d, %d, %d.', self.dock_servo_state[2], self.dock_servo_state[3], self.dock_servo_state[0], self.dock_servo_state[1])
            self.dock_detector.publish(dock(self.dock_servo_state[2], self.dock_servo_state[3], self.dock_servo_state[0], self.dock_servo_state[1]))
            self.rate.sleep()
