...
class CuBoat():
    ...
    def turn(self, position, state):
        ...
        self.dock_servo_state[i] = s
        
        if not self.dock_servo_state == self.dock_servo_state_last:
    ...
    def move(self, action):
        # action is a string
        if (action == "forward"):
            self.propeller(stop, stop, push_back, push_back-100)
            print("Forward.")
        if (action == "backward"):
            self.propeller(stop, stop, push_forward+100, push_forward+40)
            print("Backward.")
        if (action == "leftward"):
            self.propeller(push_back+80, push_forward, stop, stop)
            print("Leftward.")
        if (action == "rightward"):
            self.propeller(push_forward+60, push_back, stop, stop)
            print("Rightward.")
        if (action == "turnleft"):
